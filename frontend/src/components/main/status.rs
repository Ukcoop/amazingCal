...
pub fn Status(props: &StatusParams) -> Html {
    let base = "flex items-center w-full min-h-10 rounded-md mb-1 p-2";
    ...
    return match props.status.code {
        ...
        StatusCode::Loading => html! {
            <div class={format!("{} bg-blue-500/30 border-2 border-blue-500 text-blue-600 dark:text-blue-400", base)}>
                <MaterialSymbol name="hourglass"/>
                <div class="flex flex-col">{ rendered_lines }</div>
            </div>
        },
        StatusCode::Success => html! {
            <div class={format!("{} bg-green-500/30 border-2 border-green-500 text-green-600 dark:text-green-400", base)}>
                <MaterialSymbol name="check"/>
                <div class="flex flex-col">{ rendered_lines }</div>
            </div>
        },
        StatusCode::Error => html! {
            <div class={format!("{} bg-red-500/30 border-2 border-red-500 text-red-600 dark:text-red-400", base)}>
